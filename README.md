...
| `report` | state what a bundle holds against NIST AI 800-2, as a PDF | the bundle |
...
language server checks a plan as it is typed, and `validate`, `verify`, `estimate`, `grade`
and `report` take `--json`. What is not settled is the score card format, so `grade` applies whatever
